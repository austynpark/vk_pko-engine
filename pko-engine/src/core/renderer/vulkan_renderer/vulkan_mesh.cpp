...
void vulkan_render_object::upload_mesh()
{
    ...
    for (u32 i = 0; i < mesh_count; ++i)
    {
        ...
        vulkan_buffer_create(pContext, meshes[i].vertices.size() * sizeof(Vertex),
        ...
        vulkan_buffer_copy(pContext, &staging_buffer, &vertex_buffers[i],
        ...
        if (meshes[i].indices.size() > 0)
        {
            ...
            vulkan_buffer_create(pContext, meshes[i].indices.size() * sizeof(u32),
                                 VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_AUTO,
                                 VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT |
                                     VMA_ALLOCATION_CREATE_MAPPED_BIT,
            ...
            vulkan_buffer_create(
                pContext, meshes[i].indices.size() * sizeof(u32),
                VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                VMA_MEMORY_USAGE_AUTO, VMA_ALLOCATION_CREATE_DEDICATED_MEMORY_BIT,
                &index_buffers[i]);

            vulkan_buffer_copy(pContext, &index_staging_buffer, &index_buffers[i],
...
void vulkan_render_object::vulkan_render_object_destroy()
{
    ...
    {
        for (auto& texture : mesh.textures) vulkan_texture_destroy(pContext, &texture);
    }
...
std::vector<Texture> vulkan_render_object::load_material_textures(aiMaterial* mat,
                                                                  aiTextureType type,
                                                                  std::string typeName)
{
    std::vector<Texture> textures;
