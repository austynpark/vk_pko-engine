...
b8 check_physical_device_requirements(VkPhysicalDevice device, VkSurfaceKHR surface, device_requirements* requirements, queue_family_info* out_queue_family)
{
	...
	VK_CHECK(vkEnumerateDeviceExtensionProperties(device, 0, &available_extension_count, extension_properties.data()));
	...
	for (u32 i = 0; i < required_extension_count; ++i) {
		...
		for (u32 k = 0; k < available_extension_count; ++k) {
			if (strcmp(extension_properties.at(k).extensionName, requirements->extensions_name[i]) == 0) {
