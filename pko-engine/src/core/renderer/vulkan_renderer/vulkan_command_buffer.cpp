...
VkPipelineStageFlags get_pipeline_stage_flags(VkAccessFlags accessFlags,
                                              QueueType queueType) {
...
void vulkan_command_pool_create(RenderContext* context, Command* command,
  ...
  create_info.queueFamilyIndex =
      get_queue_family_index(&context->device_context, queueType);
  command->type = queueType;

  VK_CHECK(vkCreateCommandPool(context->device_context.handle, &create_info,
                               context->allocator, &command->pool));
}
...
void vulkan_command_pool_destroy(RenderContext* context, Command* command) {
  ...
  vkDestroyCommandPool(context->device_context.handle, command->pool,
                       context->allocator);
}
...
void vulkan_command_buffer_allocate(RenderContext* context, Command* command,
  ...
  VK_CHECK(vkAllocateCommandBuffers(context->device_context.handle, &alloc_info,
                                    &command->buffer));
}
...
void vulkan_command_resource_barrier(
  ...
  VkImageMemoryBarrier* imageMemoryBarriers = (VkImageMemoryBarrier*)malloc(
  ...
  for (uint32_t i = 0; i < buffBarrierCount; ++i) {
    ...
    if (buffBarrier->current_state == RESOURCE_STATE_UNORDERED_ACCESS &&
      ...
      memoryBarrier.dstAccessMask |=
          resource_state_to_access_flags(buffBarrier->new_state);
    }
  ...
  for (u32 i = 0; i < textureBarrierCount; ++i, ++imageMemoryBarrierCount) {
    ...
    VkImageMemoryBarrier* imageMemoryBarrier =
        &imageMemoryBarriers[imageMemoryBarrierCount];
  ...
  for (u32 i = 0; i < renderTargetBarrierCount;
    ...
    if (imageMemoryBarrier) {
      ...
      if (pRenderTargetBarrier->current_state ==
      ...
      imageMemoryBarrier->oldLayout = resource_state_to_vulkan_image_layout(
          pRenderTargetBarrier->current_state);
      imageMemoryBarrier->newLayout = resource_state_to_vulkan_image_layout(
          pRenderTargetBarrier->new_state);
      imageMemoryBarrier->srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
      imageMemoryBarrier->dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
      imageMemoryBarrier->image = texture->image;
  ...
  VkPipelineStageFlags srcStageMask =
      get_pipeline_stage_flags(srcAccessMask, command->type);
  VkPipelineStageFlags dstStageMask =
      get_pipeline_stage_flags(dstAccessMask, command->type);

  if (srcAccessMask != 0 || dstAccessMask != 0) {
    vkCmdPipelineBarrier(command->buffer, srcStageMask, dstStageMask, 0, 1,
                         &memoryBarrier, 0, nullptr, imageMemoryBarrierCount,
                         imageMemoryBarriers);
  }
