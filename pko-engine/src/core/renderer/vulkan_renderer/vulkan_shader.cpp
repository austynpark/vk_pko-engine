...
void vulkan_shader_destroy(RenderContext* context, Shader* pOutShader)
{
	for (u32 i = 0; i < MAX_SHADER_STAGE_COUNT; ++i)
	{
		if (pOutShader->names[i] != NULL && pOutShader->shader_modules[i] != NULL)
		{
			...
			pOutShader->shader_modules[i]->codes.clear();
			pOutShader->shader_modules[i]->code_size = 0;
			free(pOutShader->shader_modules[i]);
			pOutShader->shader_modules[i] = NULL;
			pOutShader->names[i] = NULL;
		}
	}
	...
	for (u32 r = 0; r < pOutShader->resource_count; ++r)
	{
		...
		{
			if (pOutShader->shader_resources[r].mMembers[m].name != NULL)
				delete[] pOutShader->shader_resources[r].mMembers[m].name;
		}


		if (pOutShader->shader_resources[r].mMembers != NULL)
			free(pOutShader->shader_resources[r].mMembers);
		pOutShader->shader_resources[r].mMembers = NULL;

		if (pOutShader->shader_resources[r].name != NULL)
			delete[] pOutShader->shader_resources[r].name;
		pOutShader->shader_resources[r].name = NULL;
	}
