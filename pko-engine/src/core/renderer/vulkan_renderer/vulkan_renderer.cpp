...
b8 VulkanRenderer::Init() {
  ...
  if (!load_exported_entry_points()) return false;

  if (!load_global_level_function()) return false;
  ...
  if (!load_instance_level_function()) return false;
  ...
  if (!load_device_level_function()) return false;
  ...
  for (u32 i = 0; i < MAX_FRAME; ++i) {
    ...
    VK_CHECK(vkCreateSemaphore(context.device_context.handle,
                               &semaphore_create_info, context.allocator,
                               &image_available_semaphores[i]));
    VK_CHECK(vkCreateSemaphore(context.device_context.handle,
                               &semaphore_create_info, context.allocator,
                               &ready_to_render_semaphores[i]));
    VkFenceCreateInfo fence_create_info{VK_STRUCTURE_TYPE_FENCE_CREATE_INFO};
    fence_create_info.flags = VK_FENCE_CREATE_SIGNALED_BIT;
    vkCreateFence(context.device_context.handle, &fence_create_info,
                  context.allocator, &render_fences[i]);
  }
...
void VulkanRenderer::Draw() {
  ...
  VK_CHECK(vkWaitForFences(context.device_context.handle, 1,
                           &render_fences[context.current_frame], true,
                           UINT64_MAX));
  VK_CHECK(vkResetFences(context.device_context.handle, 1,
  ...
  vulkan_command_buffer_begin(pCmd,
  ...
  if (!present_image_swapchain(
          &context, swapchain, context.device_context.present_queue,
          ready_to_render_semaphores[context.current_frame],
          context.image_index)) {
    /*
...
void VulkanRenderer::Shutdown() {
  ...
  for (u32 i = 0; i < MAX_FRAME; ++i) {
    vkDestroySemaphore(context.device_context.handle,
                       image_available_semaphores[i], context.allocator);
    vkDestroySemaphore(context.device_context.handle,
                       ready_to_render_semaphores[i], context.allocator);
    vkDestroyFence(context.device_context.handle, render_fences[i],
                   context.allocator);
  ...
  vkDestroyDescriptorPool(context.device_context.handle, context.imgui_pool,
                          context.allocator);
...
b8 VulkanRenderer::createInstance() {
  ...
  for (u32 i = 0; i < required_layer_count; ++i) {
    ...
    for (u32 l = 0; l < available_layer_count; ++l) {
      if (strcmp(required_layer_names[i],
                 available_properties.at(l).layerName) == 0) {
        std::cout << "VULKAN INSTANCE LAYER: " << required_layer_names[i]
                  << " found" << std::endl;
        is_found = true;
        break;
      }
  ...
  VK_CHECK(vkCreateInstance(&inst_create_info, context.allocator,
                            &context.instance));
...
void VulkanRenderer::createDebugUtilMessage() {
  ...
  VK_CHECK(vkCreateDebugUtilsMessengerEXT(context.instance, &debugCreateInfo,
...
b8 VulkanRenderer::createSurface() {
  ...
  const InternalState& internal_state =
      app_state_->platform_state->GetInternalState();

  createInfo.hinstance = internal_state.instance;
  createInfo.hwnd = internal_state.handle;

  VK_CHECK(vkCreateWin32SurfaceKHR(context.instance, &createInfo,
...
void VulkanRenderer::initImgui() {
  ...
  VK_CHECK(vkCreateDescriptorPool(context.device_context.handle, &pool_info,
                                  nullptr, &context.imgui_pool));
  ...
  for (u32 i = 0; i < MAX_FRAME; ++i) {
    vkDestroyCommandPool(context.device_context.handle, cmds[i].pool,
                         context.allocator);
    cmds[i].pool = VK_NULL_HANDLE;
  }
...
#define VK_DEVICE_LEVEL_FUNCTION(fun)                                       \
  if (!(fun = (PFN_##fun)vkGetDeviceProcAddr(context.device_context.handle, \
                                             #fun))) {                      \
