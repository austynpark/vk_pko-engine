...
b8 vulkan_rendertarget_create(RenderContext* context, RenderTargetDesc* desc, RenderTarget** out_render_target);
void vulkan_rendertarget_destroy(RenderContext* context, RenderTarget* render_target);

b8 vulkan_swapchain_create(RenderContext* context, const SwapchainDesc* desc, VulkanSwapchain** swapchain);
b8 vulkan_swapchain_destroy(RenderContext* context, VulkanSwapchain* swapchain);
b8 vulkan_swapchain_recreate(RenderContext* context, VulkanSwapchain** swapchain, i32 width, i32 height);
void vulkan_swapchain_get_support_info(RenderContext* context, VulkanSwapchainSupportInfo* out_support_info);

void vulkan_framebuffer_create(RenderContext* context, VulkanRenderpass* renderpass, u32 attachment_count, VkImageView* image_view, VkFramebuffer* out_framebuffer);
