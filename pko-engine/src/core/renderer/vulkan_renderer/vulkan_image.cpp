...
b8 vulkan_rendertarget_create(RenderContext* context, RenderTargetDesc* desc,
  ...
  RenderTarget* render_target = (RenderTarget*)alloc_aligned_memory(
      sizeof(RenderTarget) + desc->descriptor_type &
  ...
  textureDesc.start_state =
      !isDepth ? RESOURCE_STATE_RENDER_TARGET : RESOURCE_STATE_DEPTH_WRITE;
  ...
  textureDesc.type = VkDescriptorType(desc->descriptor_type |
  ...
  if (render_target->mip_levels > 1) {
    for (u32 i = 0; i < render_target->mip_levels; ++i) {
      ...
      VK_CHECK(vkCreateImageView(context->device_context.handle,
                                 &imgViewCreateInfo, context->allocator,
                                 &render_target->array_descriptors[i]));
    }
  }
  ...
  vulkan_command_buffer_begin(&oneTimeSubmit,
                              VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT);

  TextureBarrier textureBarrier;
  textureBarrier.current_state = RESOURCE_STATE_UNDEFINED;
  textureBarrier.new_state = desc->start_state;
  vulkan_command_resource_barrier(&oneTimeSubmit, NULL, 0, &textureBarrier, 1,
                                  NULL, 0);
...
void vulkan_rendertarget_destroy(RenderContext* context,
                                 RenderTarget* render_target) {
  vulkan_texture_destroy(context, render_target->texture);

  if (render_target->descriptor != VK_NULL_HANDLE) {
    vkDestroyImageView(context->device_context.handle,
                       render_target->descriptor, context->allocator);
  }
...
b8 vulkan_swapchain_create(RenderContext* context, const SwapchainDesc* desc,
  ...
  VulkanSwapchain* swapchain = (VulkanSwapchain*)alloc_aligned_memory(
      sizeof(VulkanSwapchain) + sizeof(RenderTarget*) * desc->image_count +
          sizeof(SwapchainDesc),
      16);
  swapchain->render_targets = (RenderTarget**)(swapchain + 1);
  swapchain->desc =
      (SwapchainDesc*)(swapchain->render_targets + desc->image_count);
  assert(swapchain);
  ...
  for (u32 i = 0; i < context->swapchain_support_info.format_count; ++i) {
    VkSurfaceFormatKHR surface_format =
        context->swapchain_support_info.surface_formats[i];
  ...
  for (u32 i = 0; i < context->swapchain_support_info.present_mode_count; ++i) {
    if (context->swapchain_support_info.present_modes.at(i) ==
  ...
  VK_CHECK(vkCreateSwapchainKHR(context->device_context.handle,
  ...
  for (uint32_t rt = 0; rt < number_of_images; ++rt) {
    ...
    vulkan_rendertarget_create(context, &renderTargetDesc,
                               &swapchain->render_targets[rt]);
  }
...
b8 vulkan_swapchain_destroy(RenderContext* context,
                            VulkanSwapchain* swapchain) {
  for (u32 i = 0; i < swapchain->image_count; ++i) {
    vulkan_rendertarget_destroy(context, swapchain->render_targets[i]);
  }

  vkDestroySwapchainKHR(context->device_context.handle, swapchain->handle,
                        context->allocator);
...
b8 vulkan_swapchain_recreate(RenderContext* context,
                             VulkanSwapchain** ppSwapchain, i32 width,
                             i32 height) {
  assert(ppSwapchain);
...
void vulkan_swapchain_get_support_info(
    RenderContext* context, VulkanSwapchainSupportInfo* out_support_info) {
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
      context->device_context.physical_device, context->surface,
      &out_support_info->surface_capabilites);

  vkGetPhysicalDeviceSurfaceFormatsKHR(context->device_context.physical_device,
                                       context->surface,
...
b8 acquire_next_image_index_swapchain(RenderContext* context,
                                      VulkanSwapchain* swapchain,
                                      u64 timeout_ns,
                                      VkSemaphore image_available_semaphore,
                                      VkFence fence, u32* image_index) {
  VkResult result = vkAcquireNextImageKHR(
      context->device_context.handle, swapchain->handle, timeout_ns,
...
void vulkan_framebuffer_create(RenderContext* context,
  ...
  VK_CHECK(vkCreateFramebuffer(context->device_context.handle, &create_info,
                               context->allocator, out_framebuffer));
}

void vulkan_framebuffer_destroy(RenderContext* context,
                                VkFramebuffer* framebuffer) {
  vkDestroyFramebuffer(context->device_context.handle, *framebuffer,
                       context->allocator);
  framebuffer = 0;
}
...
void vulkan_texture_create(RenderContext* context, TextureDesc* desc,
                           Texture** ptexture) {
  assert(context);
  assert(desc);
  assert(ptexture);
  ...
  if (texture->image == VK_NULL_HANDLE) {
    ...
    imgCreateInfo.usage =
        descriptor_type_to_vulkan_image_usage(desc->type) |
    ...
    VK_CHECK(vmaCreateImage(context->vma_allocator, &imgCreateInfo,
                            &allocCreateInfo, &texture->image, &texture->pAlloc,
                            nullptr));
  }
  ...
  viewCreateInfo.subresourceRange.aspectMask =
      format_to_vulkan_image_aspect(desc->vulkan_format);
  ...
  if (desc->type & VK_DESCRIPTOR_TYPE_STORAGE_IMAGE) {
    ...
    for (u32 i = 0; i < desc->mip_levels; ++i) {
      ...
      VK_CHECK(vkCreateImageView(context->device_context.handle,
                                 &viewCreateInfo, context->allocator,
                                 &texture->uav_descriptors[i]));
    }
  }
...
void vulkan_texture_destroy(RenderContext* context, Texture* texture) {
  ...
  if (texture->uav_descriptors) {
    for (u32 i = 0; i < texture->mip_levels; ++i) {
      vkDestroyImageView(context->device_context.handle,
                         texture->uav_descriptors[i], context->allocator);
    }
  }
...
b8 load_image_from_file(RenderContext* context, const char* file,
                        Texture* texture) {
  i32 tex_width, tex_height, tex_channels;
  stbi_uc* pixels =
      stbi_load(file, &tex_width, &tex_height, &tex_channels, STBI_rgb_alpha);
  ...
  vulkan_buffer_create(context, staging_buffer_size,
                       VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_AUTO,
                       VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT |
                           VMA_ALLOCATION_CREATE_MAPPED_BIT,
                       &staging_buffer);
  vulkan_buffer_upload(context, &staging_buffer, pixel_ptr,
                       staging_buffer_size);
  ...
  vulkan_command_buffer_begin(&one_time_submit,
                              VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT);
  ...
  vkCmdCopyBufferToImage(one_time_submit.buffer, staging_buffer.handle,
  ...
  VK_CHECK(vkQueueSubmit(context->device_context.transfer_queue, 1,
