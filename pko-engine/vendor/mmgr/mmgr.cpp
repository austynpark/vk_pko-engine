...
// ---------------------------------------------------------------------------------------------------------------------------------
// -DOC- Allocate aligned size of memory
// ---------------------------------------------------------------------------------------------------------------------------------

void* mmgrAllocateAlignedMemory(uint32_t size, uint32_t alignment, const char* srcName, uint32_t line, const char* funcName)
{
	size_t new_size = ALIGN_TO(size, alignment);
	
	void* mem_ptr = m_allocator(srcName, line, funcName, m_alloc_malloc, new_size);
...
{
	m_deallocator(srcName, line, funcName, m_alloc_free, ptr);
}
