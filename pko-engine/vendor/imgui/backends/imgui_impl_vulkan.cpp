...
//  [!] Renderer: User texture binding. Use 'VkDescriptorSet' as ImTextureID. Read the FAQ about
//  ImTextureID! See https://github.com/ocornut/imgui/pull/914 for discussions. [X] Renderer: Large
//  meshes support (64k+ vertices) with 16-bit indices.

// Important: on 32-bit systems, user texture binding is only supported if your imconfig file has
// '#define ImTextureID ImU64'. This is because we need ImTextureID to carry a 64-bit value and by
// default ImTextureID is defined as void*. To build this on 32-bit systems and support texture
// changes:
// - [Solution 1] IDE/msbuild: in "Properties/C++/Preprocessor Definitions" add 'ImTextureID=ImU64'
// (this is what we do in our .vcxproj files)
// - [Solution 2] IDE/msbuild: in "Properties/C++/Preprocessor Definitions" add
// 'IMGUI_USER_CONFIG="my_imgui_config.h"' and inside 'my_imgui_config.h' add '#define ImTextureID
// ImU64' and as many other options as you like.
// - [Solution 3] IDE/msbuild: edit imconfig.h and add '#define ImTextureID ImU64' (prefer solution
// 2 to create your own config file!)
// - [Solution 4] command-line: add '/D ImTextureID=ImU64' to your cl.exe command-line (this is what
// we do in our batch files)
...
//  2024-01-03: Vulkan: Added MinAllocationSize field in ImGui_ImplVulkan_InitInfo to workaround
//  zealous "best practice" validation layer. (#7189, #4238) 2024-01-03: Vulkan: Stoped creating
//  command pools with VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT as we don't reset them.
//  2023-11-29: Vulkan: Fixed mismatching allocator passed to vkCreateCommandPool() vs
...
//              unecessary as we create and destroy those objects in the backend.
...
//  2022-10-04: Vulkan: Added experimental ImGui_ImplVulkan_RemoveTexture() for api symetry. (#914,
...
//  reduce likehood of issues with people using VK_DYNAMIC_STATE_SCISSOR in their app without
                                 ...
                                 VkBufferUsageFlagBits usage)
{
    ...
    VkDeviceSize vertex_buffer_size_aligned =
        ((new_size - 1) / bd->BufferMemoryAlignment + 1) * bd->BufferMemoryAlignment;
                                     ...
                                     VkPipeline pipeline)
{
    ...
    if (wrb->FrameRenderBuffers == nullptr)
    {
        ...
        wrb->FrameRenderBuffers = (ImGui_ImplVulkanH_FrameRenderBuffers*)IM_ALLOC(
    ...
    if (draw_data->TotalVtxCount > 0)
    {
        ...
        VkResult err = vkMapMemory(v->Device, rb->VertexBufferMemory, 0, rb->VertexBufferSize, 0,
                                   (void**)&vtx_dst);
        check_vk_result(err);
        err = vkMapMemory(v->Device, rb->IndexBufferMemory, 0, rb->IndexBufferSize, 0,
                          (void**)&idx_dst);
        check_vk_result(err);
...
// You probably never need to call this, as it is called by ImGui_ImplVulkan_CreateFontsTexture()
...
void ImGui_ImplVulkan_DestroyDeviceObjects()
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    ImGui_ImplVulkanH_DestroyWindowRenderBuffers(v->Device, &bd->MainWindowRenderBuffers,
                                                 v->Allocator);
    ImGui_ImplVulkan_DestroyFontsTexture();
...
bool ImGui_ImplVulkan_LoadFunctions(PFN_vkVoidFunction (*loader_func)(const char* function_name,
                                                                      void* user_data),
                                    void* user_data)
{
    // Load function pointers
    // You can use the default Vulkan loader using:
    //      ImGui_ImplVulkan_LoadFunctions([](const char* function_name, void*) { return
...
bool ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info, VkRenderPass render_pass)
{
    ...
    if (info->UseDynamicRendering)
    {
        ...
        ImGuiImplVulkanFuncs_vkCmdEndRenderingKHR = reinterpret_cast<PFN_vkCmdEndRenderingKHR>(
...
#else
        IM_ASSERT(0 &&
                  "Can't use dynamic rendering when neither VK_VERSION_1_3 or "
...
// you own custom tailored code to do equivalent work. (The ImGui_ImplVulkanH_XXX functions do not
                                                         ...
                                                         VkColorSpaceKHR request_color_space)
{
    ...
    // VK_IMAGE_CREATE_MUTABLE_BIT was set at image creation Assuming that the default behavior is
    // without setting this bit, there is no need for separate Swapchain image and image view format
                                             ...
                                             uint32_t min_image_count)
{
    ...
    // We don't use ImGui_ImplVulkanH_DestroyWindow() because we want to preserve the old swapchain
    ...
    // Create Swapchain
    {
        ...
        wd->Frames =
            (ImGui_ImplVulkanH_Frame*)IM_ALLOC(sizeof(ImGui_ImplVulkanH_Frame) * wd->ImageCount);
        wd->FrameSemaphores = (ImGui_ImplVulkanH_FrameSemaphores*)IM_ALLOC(
            sizeof(ImGui_ImplVulkanH_FrameSemaphores) * wd->ImageCount);
    ...
    if (wd->UseDynamicRendering == false)
    {
        ...
        // VK_SAMPLE_COUNT_1_BIT, &wd->Pipeline, bd->Subpass);
    }
                                                  ...
                                                  const VkAllocationCallbacks* allocator)
{
    for (uint32_t n = 0; n < buffers->Count; n++)
        ImGui_ImplVulkanH_DestroyFrameRenderBuffers(device, &buffers->FrameRenderBuffers[n],
                                                    allocator);
